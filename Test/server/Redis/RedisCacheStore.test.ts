...
import { RedisCacheStore } from '@nexoralShared/Redis/RedisCacheStore';
...
describe('RedisCacheStore.set', () => {
  ...
  it('floors ttl <= 0 to 1s instead of skipping the write or caching forever', async () => {
    const { client, store } = setup();
    await store.set('k1', 'v', 0);
    expect(client.setEx).toHaveBeenCalledWith('k1', 1, 'v');
    await store.set('k2', 'v', -5);
    expect(client.setEx).toHaveBeenCalledWith('k2', 1, 'v');
  });
