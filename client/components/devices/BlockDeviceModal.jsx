...
export default function BlockDeviceModal({ onClose, onSave, deviceIP }) {
  ...
  // Fetch Domain Groups on mount
  useEffect(() => {
    const fetchGroups = async () => {
      setLoadingGroups(true);
      try {
        const domainGroupsResponse = await api.getDomainGroups();
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-t border-slate-200 flex items-center justify-between">
          ...
          >
            ...
            <span>{step === 2 ? (loading ? 'Creating...' : 'Create Policy') : 'Next'}</span>
          </button>
        </div>
      </div>
    </div>
