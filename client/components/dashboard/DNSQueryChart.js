...
export default function DNSQueryChart({ analytics }) {
  if (!analytics) {
    return (
      <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border-2)] p-6">
        ...
        <div className="text-center py-8 text-[var(--text-5)] text-sm">Loading...</div>
      </div>
    );
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-[var(--card-bg)] px-4 py-3 rounded-lg border border-[var(--border-4)] shadow-xl">
          <p className="font-semibold text-[var(--text-1)] text-sm">{payload[0].name}</p>
          <p className="text-xs text-[var(--text-3)] mt-1">
            Queries: <span className="font-medium text-[var(--text-2)]">{payload[0].value.toLocaleString()}</span>
          </p>
  ...
  return (
    <div className="bg-[var(--card-bg)] rounded-xl border border-[var(--border-2)] p-6">
      <h2 className="text-base font-bold text-[var(--text-1)] mb-6 flex items-center">
        <svg className="w-5 h-5 mr-2 text-[var(--blue)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        </svg>
        DNS Query Distribution (24h)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col justify-center space-y-3">
          {data.map((item, index) => (
            <div key={index} className="flex items-center justify-between p-3 rounded-lg hover:bg-[var(--surface-1)] transition-colors">
              <div className="flex items-center">
                <div className="w-3 h-3 rounded-full mr-3 flex-shrink-0" style={{ backgroundColor: item.color }} />
                <span className="font-medium text-[var(--text-2)] text-sm">{item.name}</span>
              </div>
              <div className="text-right">
                <div className="text-base font-bold text-[var(--text-1)]">{item.value.toLocaleString()}</div>
                <div className="text-xs text-[var(--text-5)]">{item.percentage.toFixed(2)}%</div>
              </div>
            </div>
          ))}
          ...
          <div className="pt-3 border-t border-[var(--border)]">
            <div className="flex items-center justify-between p-3 bg-[rgba(91,140,255,0.07)] rounded-lg border border-[rgba(91,140,255,0.15)]">
              <span className="font-semibold text-[var(--text-2)] text-sm">Total Queries</span>
