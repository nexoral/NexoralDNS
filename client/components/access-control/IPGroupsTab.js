  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-[var(--text-1)]">IP Groups</h3>
          <p className="text-sm text-[var(--text-3)] mt-1">Organize IP addresses into groups for easier management</p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-[var(--text-3)]">Loading IP groups...</p>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-12 bg-[var(--bg)] rounded-lg border-2 border-dashed border-[var(--border-2)]">
          ...
          <h3 className="text-lg font-medium text-[var(--text-1)] mb-2">No IP groups found</h3>
          <p className="text-[var(--text-3)] mb-4">Create your first IP group to organize IP addresses</p>
              ...
              <div className="bg-[var(--bg)] rounded p-3 max-h-32 overflow-y-auto">
                <div className="text-xs text-[var(--text-3)] space-y-1">
                  ...
                  {group.ipAddresses?.length > 5 && (
                    <div className="text-[var(--text-5)] italic">+ {group.ipAddresses.length - 5} more</div>
                  )}
                </div>
              </div>
            </div>
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--card-bg)] rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-2)] mb-2">Description (Optional)</label>
            <input
              type="text"
            ...
            {formData.ipAddresses.length > 0 && (
              <div className="space-y-2 max-h-60 overflow-y-auto p-3 bg-[var(--bg)] rounded-lg">
                {formData.ipAddresses.map((ip, idx) => (
                  <div key={idx} className="flex items-center justify-between p-2 bg-[var(--card-bg)] rounded border border-[var(--border-2)]">
                    <span className="text-sm font-mono text-[var(--text-2)]">{ip}</span>
                    <button
                      onClick={() => removeIP(ip)}
