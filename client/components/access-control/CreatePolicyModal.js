      ...
      <div className="bg-[var(--card-bg)] rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 border-b border-[var(--border-2)]">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-[var(--text-1)]">Create Access Control Policy</h2>
            <button
              onClick={onClose}
            ...
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center flex-1">
                <div className={`flex items-center justify-center w-8 h-8 rounded-full font-semibold text-sm ${
                  step >= s ? 'bg-blue-600 text-white' : 'bg-slate-200 text-[var(--text-3)]'
                }`}>
                  {s}
                </div>
                {s < 3 && (
        ...
        <div className="p-6 min-h-[450px]">
          ...
          {step === 1 && (
            <div>
              <h3 className="text-lg font-semibold text-[var(--text-1)] mb-2">WHO should be blocked?</h3>
              <p className="text-sm text-[var(--text-3)] mb-4">Select which users or devices this policy applies to</p>

              <div className="space-y-4">
                {/* Single IP */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.targetType === 'single_ip' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    ...
                    />
                    <div className="ml-3">
                      <span className="font-medium text-[var(--text-1)]">Single IP Address</span>
                      <p className="text-sm text-[var(--text-3)]">Block a specific device by IP</p>
                    </div>
                  </div>
                ...
                {/* Multiple IPs */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.targetType === 'multiple_ips' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                    ...
                    />
                    <div className="ml-3">
                      <span className="font-medium text-[var(--text-1)]">Multiple IP Addresses</span>
                      <p className="text-sm text-[var(--text-3)]">Block several specific devices</p>
                    </div>
                  </div>
                      ...
                      {formData.targetIPs.length > 0 && (
                        <div className="space-y-2">
                          {formData.targetIPs.map((ip, idx) => (
                            <div key={idx} className="flex items-center justify-between p-2 bg-[var(--bg)] rounded border border-[var(--border-2)]">
                              <span className="text-sm text-[var(--text-2)]">{ip}</span>
                              <button onClick={() => removeIP(ip)} className="text-[var(--red)] hover:text-[var(--red)]">
                ...
                {/* Single IP Group */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.targetType === 'ip_group' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                    ...
                    />
                    <div className="ml-3">
                      <span className="font-medium text-[var(--text-1)]">IP Group</span>
                      <p className="text-sm text-[var(--text-3)]">Block a pre-defined group of devices</p>
                    </div>
                  </div>
                ...
                {/* Multiple IP Groups */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.targetType === 'multiple_ip_groups' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                    ...
                    />
                    <div className="ml-3">
                      <span className="font-medium text-[var(--text-1)]">Multiple IP Groups</span>
                      <p className="text-sm text-[var(--text-3)]">Block multiple pre-defined device groups</p>
                    </div>
                  </div>
                  {formData.targetType === 'multiple_ip_groups' && (
                    <div className="mt-3 space-y-2 max-h-48 overflow-y-auto">
                      {loadingGroups ? (
                        ...
                        <div className="text-center py-4 text-[var(--text-3)]">No IP groups available</div>
                      ) : (
                        ipGroups.map((group) => (
                          <label
                            key={group._id}
                ...
                {/* All Users */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.targetType === 'all' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                    ...
                    />
                    <div className="ml-3">
                      <span className="font-medium text-[var(--text-1)]">All Users</span>
                      <p className="text-sm text-[var(--text-3)]">Apply to all devices on the network</p>
                    </div>
                  </div>
                </label>
              </div>
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-sm text-[var(--text-3)] mb-4">Choose what content or services to block</p>

              <div className="space-y-4">
                {/* Specific Domains */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.blockType === 'specific_domains' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                      ...
                      <div className="space-y-2">
                        ...
                        <label className="flex items-center space-x-2 text-sm text-[var(--text-2)] cursor-pointer">
                          ...
                          <span>
                            ...
                            <span className="text-[var(--text-5)] ml-1">
                ...
                {/* Single Domain Group */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.blockType === 'domain_group' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                ...
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.blockType === 'multiple_domain_groups' ? 'border-blue-500 bg-[rgba(91,140,255,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
                ...
                {/* Full Internet */}
                <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${
                  formData.blockType === 'full_internet' ? 'border-red-500 bg-[rgba(255,96,113,0.07)]' : 'border-[var(--border-2)] hover:border-[var(--border-4)]'
                }`}>
                  <div className="flex items-center">
                    <input
                      type="radio"
