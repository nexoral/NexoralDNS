...
export default function PoliciesTab() {
  ...
  const getTypeBadge = (type) => {
    const badges = {
      user_domain: { label: 'User → Domain', color: 'bg-[rgba(91,140,255,0.12)] text-[var(--blue)] border-blue-300' },
      user_internet: { label: 'User → Internet', color: 'bg-[rgba(255,96,113,0.12)] text-[var(--red)] border-red-300' },
      domain_all: { label: 'Domain → All', color: 'bg-purple-100 text-[var(--purple)] border-purple-300' },
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-[var(--text-1)]">Blocking Policies</h3>
          <p className="text-sm text-[var(--text-3)] mt-1">Create and manage access control policies</p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-[var(--text-3)]">Loading policies...</p>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-12 bg-[var(--bg)] rounded-lg border-2 border-dashed border-[var(--border-2)]">
          <div className="text-6xl mb-4">🛡️</div>
          <h3 className="text-lg font-medium text-[var(--text-1)] mb-2">No policies found</h3>
          <p className="text-[var(--text-3)] mb-4">Create your first blocking policy to get started</p>
        ...
        <div className="space-y-3">
          {filteredPolicies.map((policy) => {
            ...
            return (
              <div
                key={policy._id}
                className="bg-[var(--card-bg)] border border-[var(--border-2)] rounded-lg p-5 hover:shadow-md transition-shadow"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <h4 className="text-base font-semibold text-[var(--text-1)]">{policy.policyName}</h4>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${badge.color}`}>
                        {badge.label}
                      </span>
                    </div>
                    <div className="flex items-center space-x-6 text-sm text-[var(--text-3)]">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <button
                      ...
                      className="p-2 text-[var(--red)] hover:bg-[rgba(255,96,113,0.07)] rounded-md transition-colors"
                    >
