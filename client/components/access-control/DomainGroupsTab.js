  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-[var(--text-1)]">Domain Groups</h3>
          <p className="text-sm text-[var(--text-3)] mt-1">Organize domains into groups for easier blocking</p>
        </div>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="mt-4 text-[var(--text-3)]">Loading domain groups...</p>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-12 bg-[var(--bg)] rounded-lg border-2 border-dashed border-[var(--border-2)]">
          ...
          <h3 className="text-lg font-medium text-[var(--text-1)] mb-2">No domain groups found</h3>
          <p className="text-[var(--text-3)] mb-4">Create your first domain group to organize domains</p>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-[var(--text-1)]">{group.name}</h4>
                    {group.description && (
                      <p className="text-xs text-[var(--text-5)]">{group.description}</p>
                    )}
                  </div>
                </div>
              ...
              {/* Domain Preview */}
              <div className="bg-[var(--bg)] rounded p-3 max-h-32 overflow-y-auto">
                <div className="text-xs text-[var(--text-3)] space-y-1">
                    ...
                    return (
                      <div key={idx} className="flex items-center justify-between">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded ml-2 ${
                          isWildcard ? 'bg-[rgba(91,140,255,0.12)] text-[var(--blue)]' : 'bg-slate-200 text-[var(--text-3)]'
                        }`}>
                          {isWildcard ? '🌐' : '🎯'}
                        </span>
                      </div>
                  ...
                  {group.domains?.length > 5 && (
                    <div className="text-[var(--text-5)] italic">+ {group.domains.length - 5} more</div>
                  )}
                </div>
              </div>
            </div>
...
function DomainGroupModal({ group, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--card-bg)] rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-[var(--border-2)]">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-[var(--text-6)] hover:text-[var(--text-3)]">
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[var(--text-2)] mb-2">Group Name</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-2)] mb-2">Icon</label>
                ...
                <button
                  ...
                  className={`text-2xl p-2 rounded-lg transition-colors ${
                    formData.icon === icon
                      ...
                      : 'bg-[var(--surface-3)] hover:bg-slate-200'
                  }`}
                >
                  {icon}
                </button>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text-2)] mb-2">Domains</label>
            <div className="space-y-2 mb-3">
              <input
                type="text"
            ...
            {formData.domains.length > 0 && (
              <div className="space-y-2 max-h-60 overflow-y-auto p-3 bg-[var(--bg)] rounded-lg">
                {formData.domains.map((domainEntry, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center justify-between p-3 bg-[var(--card-bg)] rounded border border-[var(--border-2)]">
                      <div className="flex-1">
                        <span className="text-sm font-mono font-medium text-[var(--text-2)]">{domain}</span>
                      ...
                      <button
                        ...
                        className="text-[var(--red)] hover:text-[var(--red)] ml-2"
                      >
        ...
        <div className="p-6 border-t border-[var(--border-2)] flex justify-end space-x-3">
          <button
            onClick={onClose}
            className="px-6 py-2 text-[var(--text-3)] hover:text-[var(--text-1)] font-medium"
          >
            Cancel
          </button>
          <button
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--card-bg)] rounded-xl shadow-2xl max-w-md w-full">
        <div className="p-6">
          ...
          <h3 className="text-lg font-semibold text-[var(--text-1)] text-center mb-2">
            Delete Domain Group
          </h3>
          <p className="text-[var(--text-3)] text-center mb-4">
            Are you sure you want to delete <span className="font-semibold">"{group.name}"</span>?
          </p>
          <p className="text-sm text-[var(--text-5)] text-center mb-4">
          ...
          {error && (
            <div className="mb-4 p-3 bg-[rgba(255,96,113,0.07)] border border-red-200 rounded-lg">
              ...
              {policiesInUse.length > 0 && (
                <div className="mt-3">
                  <p className="text-xs text-[var(--red)] font-semibold mb-2">Policies using this group:</p>
                  <ul className="space-y-1">
                    {policiesInUse.map((policy) => (
                      <li key={policy.id} className="text-xs text-[var(--red)] flex items-center">
                  ...
                  <p className="text-xs text-[var(--red)] mt-3 italic">Please remove this group from these policies before deleting.</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex space-x-3">
            <button
              onClick={onClose}
              disabled={deleting}
              className="flex-1 px-4 py-2 text-[var(--text-3)] hover:text-[var(--text-1)] font-medium disabled:opacity-50"
            >
              Cancel
            </button>
            <button
